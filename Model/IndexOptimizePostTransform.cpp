...
inline float FindVertexValenceScore(size_t numActiveTris) { return s_vertexValenceScores[numActiveTris]; }
...
template <typename IndexType> struct IndexSortCompareIndexed
{
...
struct FaceValenceSort
{
    ...
    FaceValenceSort(const OptimizeVertexData *vertexData) : _vertexData(vertexData) {}
...
void OptimizeFaces(const SrcIndexType *indexList, size_t indexCount, DstIndexType *newIndexList, size_t lruCacheSize)
{
    ...
    std::unique_ptr<OptimizeVertexData[]> vertexDataList(
        new OptimizeVertexData[indexCount]); // upper bounds on size is indexCount
    ...
    for (uint32_t i = 0; i < indexCount; i += 3)
    {
        ...
        for (uint32_t v = 0; v < 3; ++v)
        {
            ...
            // need to re-sort the faces that use this vertex, as their score will change due to activeFaceListSize
            // shrinking
...
template void OptimizeFaces<uint16_t, uint16_t>(const uint16_t *indexList, size_t indexCount, uint16_t *newIndexList,
                                                size_t lruCacheSize);
template void OptimizeFaces<uint32_t, uint16_t>(const uint32_t *indexList, size_t indexCount, uint16_t *newIndexList,
                                                size_t lruCacheSize);
template void OptimizeFaces<uint32_t, uint32_t>(const uint32_t *indexList, size_t indexCount, uint32_t *newIndexList,
                                                size_t lruCacheSize);
