...
void glTF::Asset::ProcessAccessors(json &accessors)
{
    ...
    for (json::iterator it = accessors.begin(); it != accessors.end(); ++it)
    {
        ...
        char type[8] = {0};
        strncpy(type, thisAccessor.at("type").get<std::string>().c_str(), 8);
...
void glTF::Asset::ProcessMeshes(json &meshes, json &accessors)
{
    ...
    for (json::iterator meshIt = meshes.begin(); meshIt != meshes.end(); ++meshIt, ++curMesh)
    {
        ...
        for (json::iterator primIt = primitives.begin(); primIt != primitives.end(); ++primIt, ++curSubMesh)
        {
            ...
            // prim.mode = (uint16_t)vk::PrimitiveTopology::eTriangleList;
...
{
    // IEEE754 half float defined as SXXXXXMM MMMMMMMM, which exponent bias is 15;
    // float defined as SXXXXXXX XMMMMMMM MMMMMMMM MMMMMMMM, which exponent bias is 127
    // kF32toF16 convert exponent bias by scaling 2^-112
    const float kF32toF16 = (1.0 / (1ull << 56)) * (1.0 / (1ull << 56)); // 2^-112
    union {
        float f;
        uint32_t u;
    } x;
    x.f = glm::clamp(f, 0.0f, 1.0f) * kF32toF16;
    return x.u >> 13; // cut 13 fractions out to meet half float
}
    ...
    for (json::iterator it = materials.begin(); it != materials.end(); ++it)
    {
        ...
        if (thisMaterial.find("pbrMetallicRoughness") != thisMaterial.end())
        {
            ...
            if (metallicRoughness.find("baseColorTexture") != metallicRoughness.end())
                material.baseColorUV =
                    ReadTextureInfo(metallicRoughness.at("baseColorTexture"), material.textures[Material::kBaseColor]);
        ...
        if (thisMaterial.find("occlusionTexture") != thisMaterial.end())
            material.occlusionUV =
                ReadTextureInfo(thisMaterial.at("occlusionTexture"), material.textures[Material::kOcclusion]);

        if (thisMaterial.find("emissiveTexture") != thisMaterial.end())
            material.emissiveUV =
                ReadTextureInfo(thisMaterial.at("emissiveTexture"), material.textures[Material::kEmissive]);

        if (thisMaterial.find("normalTexture") != thisMaterial.end())
            material.normalUV = ReadTextureInfo(thisMaterial.at("normalTexture"), material.textures[Material::kNormal]);

        m_materials.push_back(material);
    }
}

bool ReadFile(const string &fileName, void *Dest, size_t Size)
{
...
void glTF::Asset::ProcessBuffers(json &buffers, ByteArray chunk1bin)
{
    ...
    for (json::iterator it = buffers.begin(); it != buffers.end(); ++it)
    {
        ...
        if (thisBuffer.find("uri") != thisBuffer.end())
        {
            ...
            string filepath = m_basePath + string(uri.begin(), uri.end());

            ByteArray ba = ReadFileSync(filepath);
            ASSERT(ba->size() > 0, "Missing bin file %s", filepath.c_str());
...
//     {
//     default:
//         ERROR("Unexpected sampler wrap mode");
//     case 33071:
...
vk::SamplerAddressMode GLtoVKSamplerAddressMode(int32_t glWrapMode)
{
    switch (glWrapMode)
    {
    default:
...
D3D12_FILTER GLtoD3DTextureFilterMode( int32_t magFilter, int32_t minFilter )
{
    ...
    {
    case 9728: //nearest
    case 9984: return linearMag ? D3D12_FILTER_MIN_POINT_MAG_LINEAR_MIP_POINT :
D3D12_FILTER_MIN_MAG_MIP_POINT;//nearest_mipmap_nearest case 9729: //linear case 9987: return linearMag ?
D3D12_FILTER_MIN_MAG_MIP_LINEAR : D3D12_FILTER_MIN_LINEAR_MAG_POINT_MIP_LINEAR;//linear_mipmap_linear case 9985: return
linearMag ? D3D12_FILTER_MIN_MAG_LINEAR_MIP_POINT : D3D12_FILTER_MIN_LINEAR_MAG_MIP_POINT;//linear_mipmap_nearest case
9986: break;//nearest_mipmap_linear
    }
}
*/
...
void glTF::Asset::ProcessSamplers(json &samplers)
{
    ...
    for (json::iterator it = samplers.begin(); it != samplers.end(); ++it)
    {
        ...
        sampler.anisotropic = VK_TRUE;
        sampler.filter = vk::Filter::eLinear;
        sampler.wrapS = vk::SamplerAddressMode::eRepeat;
        sampler.wrapT = vk::SamplerAddressMode::eRepeat;
        ...
        */
        ...
        {
            sampler.wrapS = GLtoVKSamplerAddressMode(thisSampler.at("wrapS"));
        }
        if (thisSampler.find("wrapT") != thisSampler.end())
        {
            sampler.wrapT = GLtoVKSamplerAddressMode(thisSampler.at("wrapT"));
        }
    }
}
...
void glTF::Asset::Parse(const std::string &filepath)
{
    ...
    std::string fileExt = Utility::ToLower(Utility::GetFileExtension(filepath));
    ...
    if (fileExt == "glb")
    {
        ...
        gltfFile = make_shared<vector<uint8_t>>(chunk0Length + 1);
        ...
        chunk1Bin = make_shared<vector<uint8_t>>(chunk1Length);
    ...
    else
    {
        ...
        chunk1Bin = make_shared<vector<uint8_t>>(0);
    }
