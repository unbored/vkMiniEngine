...
public:
    ...
    void SetFont(const std::string &fontName, float TextSize = 0.0f);

    // Resize the view space.  This determines the coordinate space of the cursor position and font size.  You
    // can always set the view size to the same dimensions regardless of actual display resolution.  It is
    // assumed, however, that the aspect ratio of this virtual coordinate system matches the actual display
    // aspect ratio.
    ...
    {
        glm::vec4 ViewportTransform;
        float NormalizeX, NormalizeY, TextSize;
        float Scale, DstBorder;
        uint32_t SrcBorder;
    };

    struct alignas(16) FragShaderParams
    {
        Color TextColor;
...
private:
    ...
    std::vector<TextVert> FillVertexBuffer(const std::string &str);
    ...
    FragShaderParams m_FSParams;
    bool m_VSConstantBufferIsStale; // Tracks when the CB needs updating
    bool m_FSConstantBufferIsStale; // Tracks when the CB needs updating
