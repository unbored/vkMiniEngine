...
void VariableGroup::LoadSettingsFromFile(FILE *file)
{
    for (auto iter = m_Children.begin(); iter != m_Children.end(); ++iter)
    {
        ...
        if (subGroup != nullptr)
        {
            ...
            fscanf(file, "%*s %[^\n]", skippedLines, (int)sizeof(skippedLines));
...
EngineVar *VariableGroup::FirstVariable(void) { return m_Children.size() == 0 ? nullptr : m_Children.begin()->second; }
...
EngineVar::EngineVar(const std::string &path, ActionCallback pfnCallback)
    : m_GroupPtr(nullptr), m_ActionCallback(pfnCallback)
{
...
BoolVar::BoolVar(const std::string &path, bool val, ActionCallback pfnCallback) : EngineVar(path, pfnCallback)
{
    m_Flag = val;
}

void BoolVar::DisplayValue(TextContext &Text) const { Text.DrawFormattedString("[%c]", m_Flag ? 'X' : '-'); }

std::string BoolVar::ToString(void) const { return m_Flag ? "on" : "off"; }
...
void BoolVar::SetValue(FILE *file, const std::string &setting)
{
    ...
    fscanf(file, pattern.c_str(), valstr, sizeof(valstr));

    // Look for one of the many affirmations
#ifdef _MSC_VER
    m_Flag = (0 == _stricmp(valstr, "1") || 0 == _stricmp(valstr, "on") || 0 == _stricmp(valstr, "yes") ||
              0 == _stricmp(valstr, "true"));
#else
    m_Flag = (0 == strcasecmp(valstr, "1") || 0 == strcasecmp(valstr, "on") || 0 == strcasecmp(valstr, "yes") ||
              0 == strcasecmp(valstr, "true"));
#endif
}

NumVar::NumVar(const std::string &path, float val, float minVal, float maxVal, float stepSize,
               ActionCallback pfnCallback)
...
void NumVar::DisplayValue(TextContext &Text) const { Text.DrawFormattedString("%-11f", m_Value); }

std::string NumVar::ToString(void) const
{
    char buf[128];
    snprintf(buf, 128, "%f", m_Value);
    return buf;
}
...
void NumVar::SetValue(FILE *file, const std::string &setting)
{
    ...
    if (fscanf(file, scanString.c_str(), &valueRead))
        *this = valueRead;
}
...
ExpVar::operator float() const { return exp2f(m_Value); }

void ExpVar::DisplayValue(TextContext &Text) const { Text.DrawFormattedString("%-11f", (float)*this); }

std::string ExpVar::ToString(void) const
{
    char buf[128];
    snprintf(buf, 128, "%f", (float)*this);
    return buf;
}
...
void IntVar::DisplayValue(TextContext &Text) const { Text.DrawFormattedString("%-11d", m_Value); }

std::string IntVar::ToString(void) const
{
    char buf[128];
    snprintf(buf, 128, "%d", m_Value);
    return buf;
}
...
void EnumVar::DisplayValue(TextContext &Text) const { Text.DrawString(m_EnumLabels[m_Value]); }

std::string EnumVar::ToString(void) const { return m_EnumLabels[m_Value]; }
...
void EnumVar::SetValue(FILE *file, const std::string &setting)
{
    ...
    if (fscanf(file, scanString.c_str(), valueRead, sizeof(valueRead)) == 1)
    {
...
DynamicEnumVar::DynamicEnumVar(const std::string &path, ActionCallback pfnCallback) : EngineVar(path, pfnCallback)
{
    m_EnumCount = 0;
    m_Value = 0;
}

void DynamicEnumVar::DisplayValue(TextContext &Text) const { Text.DrawString(m_EnumLabels[m_Value]); }
...
void CallbackTrigger::SetValue(FILE *file, const std::string &setting)
{
    ...
    std::string scanString = "\n" + setting + ": %[^\n]";
    char skippedLines[100];
    fscanf(file, scanString.c_str(), skippedLines, sizeof(skippedLines));
}
...
void HandleDigitalButtonPress(GameInput::DigitalInput button, float timeDelta, std::function<void()> action)
{
    ...
    if (glm::floor(durationHeld * timeStretch) > glm::floor(oldDuration * timeStretch))
        action();
}

void EngineTuning::Update(float frameTime)
{
    if (GameInput::IsFirstPressed(GameInput::kBackButton) || GameInput::IsFirstPressed(GameInput::kKey_back))
    ...
    HandleDigitalButtonPress(GameInput::kDPadDown, frameTime,
                             [] { sm_SelectedVariable = sm_SelectedVariable->NextVar(); });
    HandleDigitalButtonPress(GameInput::kDPadUp, frameTime,
                             [] { sm_SelectedVariable = sm_SelectedVariable->PrevVar(); });
    ...
    HandleDigitalButtonPress(GameInput::kKey_down, frameTime,
                             [] { sm_SelectedVariable = sm_SelectedVariable->NextVar(); });
    HandleDigitalButtonPress(GameInput::kKey_up, frameTime,
                             [] { sm_SelectedVariable = sm_SelectedVariable->PrevVar(); });

    if (GameInput::IsFirstPressed(GameInput::kAButton) || GameInput::IsFirstPressed(GameInput::kKey_return))
    {
...
void EngineTuning::Display(GraphicsContext &Context, float x, float y, float w, float h)
{
    ...
    else
    {
        ...
        int sx = (int)glm::floor(x * hScale);
        int sy = (int)glm::floor(y * vScale);
        uint32_t sw = (uint32_t)glm::ceil((x + w) * hScale);
        uint32_t sh = (uint32_t)glm::ceil((y + h) * vScale);
        Context.SetScissor(sx, sy, sw, sh);
...
void EngineTuning::RegisterVariable(const std::string &path, EngineVar &var)
{
    if (s_UnregisteredCount >= 0)
    {
        ...
        strcpy(s_UnregisteredPath[Idx], path.c_str());
...
bool EngineTuning::IsFocused(void) { return sm_IsVisible; }
