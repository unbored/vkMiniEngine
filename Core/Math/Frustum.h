...
namespace Math
{
    ...
    public:
        ...
        glm::vec3         GetFrustumCorner( CornerID id ) const   { return m_FrustumCorners[id]; }
        ...
        friend Frustum  operator* ( const glm::mat4& xform, const Frustum& frustum );				// Slowest (and most general)
    ...
    private:
        ...
        glm::vec3 m_FrustumCorners[8];		// the corners of the frustum
    ...
    inline bool Frustum::IntersectBoundingBox(const AxisAlignedBox& aabb) const
    {
        for (int i = 0; i < 6; ++i)
        {
            ...
            glm::vec3 farCorner = glm::mix(aabb.GetMin(), aabb.GetMax(), glm::greaterThan(p.GetNormal(), glm::vec3(0)));
    ...
    inline Frustum operator* ( const AffineTransform& xform, const Frustum& frustum )
    {
        ...
        glm::mat4 XForm = glm::transpose(glm::inverse(glm::mat4(xform)));

        for (int i = 0; i < 6; ++i)
            result.m_FrustumPlanes[i] = BoundingPlane(XForm * glm::vec4(frustum.m_FrustumPlanes[i]));

        return result;
    }

    inline Frustum operator* ( const glm::mat4& mtx, const Frustum& frustum )
    {
        Frustum result;

        for (int i = 0; i < 8; ++i)
            result.m_FrustumCorners[i] = glm::vec3( glm::mat3(mtx) * frustum.m_FrustumCorners[i] );

        glm::mat4 XForm = glm::transpose(glm::inverse(mtx));
